...
export class AuthService {
  ...
  private readonly mockUsers: Record<string, { userId: string; roles: string[]; permissions: string[] }> = {
    'user1': {
      userId: 'mock-001',
      roles: ['RBOFFORDERS', 'RBOPAYMENT', 'RBOPAYMENTVIEW', 'ADMIN'],
      permissions: [
   ...
   * Note: This bypasses httpOnly cookies and should only be used for development.
   * @param username - One of: user1 (admin), user2 (orders), user3 (payments)
   */
  mockLogin(username: string): void {
    const mockUser = this.mockUsers[username.toLowerCase()];
    if (!mockUser) {
      console.error(`Unknown mock user: ${username}. Available: user1, user2, user3`);
      return;
    }
    ...
    this.setUser(user);
    console.log(`Mock login successful for ${username}`, { roles: mockUser.roles, permissions: mockUser.permissions });
  }

  /** Get available mock users for UI display */
  getMockUsers(): { username: string; description: string; roles: string[] }[] {
    return [
      { username: 'user1', description: 'Admin (all access)', roles: this.mockUsers['user1'].roles },
      { username: 'user2', description: 'Orders only', roles: this.mockUsers['user2'].roles },
      { username: 'user3', description: 'Payments only', roles: this.mockUsers['user3'].roles }
    ];
  }
}
